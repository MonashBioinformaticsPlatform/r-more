...
This day is a very fast tour of some ways R as currently used. The point is to get a taste of what exists.

R continues to evolve as better ways to work with data are found. In particular, "Tidyverse" packages have recently revolutionized R, including `dplyr`, `tidyr`, and `ggplot2`. These are mostly written by [Hadley Wickham](http://hadley.nz/).

Not always the most consistent environment in which to work with data, but by far the most comprehensive.
...
For example in biology you may want to join together many different views of a process: DNA, RNA, epigenetics, proteins, metabolome, cell morphology, ...
...
See also: [The "R for Data Science" book](http://r4ds.had.co.nz/introduction.html)
...
* Shrinks large data sets for more manageable visualization.
    * If nothing else, data can always be subsampled.
...
If every step of your analysis can run as a script:

* you have a complete record of what you have done
* it's easy to feed in some test data to check that it is working as expected
...
It's not science until it's shared and reproducable.
