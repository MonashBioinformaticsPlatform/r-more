...
We [saw ggplot2 in the introductory R day](https://monashbioinformaticsplatform.github.io/r-intro/ggplot.html). Recall that we could assign columns of a data frame to aesthetics--x and y position, color, etc--and then add "geom"s to draw the data.
...
`dplyr` gives us a collection of convenient "verbs" for manipulating data frames. The name is a contraction of "Data frame apPLYeR", as some of these verbs have a similar flavour to apply/lappy/tapply/etc.
...
As we used `readr` to read the data, `bigtab` is a "tibble", which is Hadley Wickham's slightly improved data frame. The `dplyr` verbs also all return their results as tibbles. You can also create tibbles explicitly with the `tibble` function. One convenient feature is that it only shows a few rows of the data frame when you print it. If you do want to see the whole table, you can use `as.data.frame`, or use the `View` function to view it in a tab.
...
The idea of adding geoms in `ggplot2` is rather like the `dplyr` pipe. `ggplot2` predates `dplyr`, and Hadley Wickham has had a progression of ideas. It will probably be possible to use `%>%` instead of `+` in `ggplot2` in the not too distant future.


## Challenge {.challenge}

In the previous challenge, we filtered and arranged `scoretab`. Rewrite this operation using the pipe.
...
`spread` is the opposite operation, spreading a column into many columns, which generally makes data less tidy but is sometimes necessary. It's a little like calling `tapply` with a function that just returns the data unaltered (`identity`).
...
Extension: Compare plots of `log_norm_count`, `norm_count`, and `count`.
