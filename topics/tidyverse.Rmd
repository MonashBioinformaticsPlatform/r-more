...
<b>Advanced:</b> `tidyr` has a number of other useful data frame manipulations. For completeness, we mention `nest` and `unnest`. These create or unpack a *list column* in a tibble, the list column containing one tibble per row. Yo, Hadley put tibbles in your tibble so you can dplyr while you dplyr. `nest` and `unnest` go well with creating or modifying list columns in a `mutate` with `lapply` or `Map`.
